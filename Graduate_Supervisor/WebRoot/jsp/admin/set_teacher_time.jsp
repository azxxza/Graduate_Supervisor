...
function initBasicGrid() {
	jQuery('#basicGrid_div').datagrid({
		...
		url : '${ctx}/admin/getTeacherTimeList?Date='
				+ new Date() + '',
		 columns : [ [
...
function submitData() {
	...
	var saveURL = "${ctx}/admin/updateTeacherTime?date="
				+ new Date() + "";
...
function confirmAdd(){
	...
	var  saveURL = "${ctx}/admin/saveTeacherTime?date="
				+ new Date() + "";
...
function delOpenTime(id){
	var delURL = "${ctx}/admin/deleteTeacherTime?id="
			+ id + "&date=" + new Date() + "";

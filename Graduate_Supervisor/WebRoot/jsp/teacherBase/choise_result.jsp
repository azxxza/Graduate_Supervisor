		...
		url : '${pageContext.request.contextPath}/teacherBase/getTeacherBaseList?Date='
		 ...
		 {field:  'detail',title:'详细信息',width:getWidth(0.1),align:'center',
       		 formatter: function(value,row,index){
				 var detail = "<a href='#' class='detailcls' style='color:blue;text-decoration:none'  onclick='detailStudent("+index+")'></a>";  
				 return detail; 
       		 } 
		 }
 ...
 * 详细信息
 */
function detailStudent(index) {
	$('#basicGrid_div').datagrid('selectRow', index);// 关键在这里
	var row = $('#basicGrid_div').datagrid('getSelected');
	var t_work_id = row.t_work_id;
	$('#win').window({
	
		content : "<iframe frameborder='0' scrolling='NO' style='width:100%;height:100%;' src='${ctx}/studentBase/detailStudent?t_work_id="
		+ t_work_id+"'></iframe>",
	});
	$('#win').window('open');
...
<body class="easyui-layout">
	...
	<div id="toolbar"  style="padding-right: 5%;">
		<a href="javascript:void(0)" class="easyui-linkbutton" iconCls="icon-export" plain="true" onclick="exportAll();">导出双选报表</a> 
	</div>
